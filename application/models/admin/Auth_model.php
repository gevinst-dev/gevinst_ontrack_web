...
class Auth_model extends CI_Model {
    ...
    public function authenticate($email, $password)
    {
        ...
        $query = $this->db->get_where('core_staff', array('email' => $email));
        ...
        if($query->num_rows() == 0) {
            ...
            if($password_valid) {
                ...
                log_staff(__('Failed authentication: incorrect password.'), $result['id']);
                return FALSE;
            }
        }
    ...
    {
        $query = $this->db->get_where('core_staff', array('password_reset_key' => $key));
    ...
    public function set_password_reset_key($email)
    {
        ...
        $this->db->update('core_staff', ["password_reset_key" => $password_reset_key]);
    ...
    public function reset_password($key, $new_password)
    {
        ...
        $item = $this->db->get_where('core_staff', array('password_reset_key' => $key))->row_array();

        $this->db->where('password_reset_key', $key);
        $this->db->update('core_staff', ['password' => $new_password, "password_reset_key" => '']);
