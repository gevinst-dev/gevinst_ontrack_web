<?php
	...
	class Datatables {
		...
		/**
		 * Sets additional column variables for editing columns conditionaly
		 *
		 * @param string $column
		 * @param string $content
		 * @param mixed $match_replacement
		...
		public function edit_column_if($column, $content, $match_replacement, $condition) {

			$this->edit_columns_if[$column][]	= array('content'	=> $content,
								'replacement'	=> $this->explode(',', $match_replacement),
								'condition'	=> $condition, );
		...
		private function produce_output($output, $charset) {
			...
			foreach ($rResult->result_array() as $row_key => $row_val) {
				...
				foreach ($this->edit_columns_if as $modkey => $modval) {


					foreach ($modval as $val) {
				
						if($val['condition'] == $aaData[$row_key][$modkey]) {
							$aaData[$row_key][($this->check_cType()) ? $modkey : array_search($modkey, $this->columns)]	= $this->exec_replace($val, $aaData[$row_key]);
						}
